"""Training script for Polymer VAE on 2D bead-spring polymer conformations.
...
Usage:
    python train.py [--data_dir data/polymer] [--output_dir outputs/polymer_vae]
"""
...
import numpy as np
import torch
from model import PolymerVAE, radius_of_gyration
...
# ---------------------------------------------------------------------------
# Preprocessing (inlined — flat philosophy)
# ---------------------------------------------------------------------------
...
def find_principal_axis(coords):
    """Find principal axis via SVD for each conformation. coords: [N, 12, 2]"""
    flat = coords.reshape(coords.shape[0], -1)  # [N, 24]
    # SVD on centered coordinates
    _, _, Vt = np.linalg.svd(flat - flat.mean(axis=0, keepdims=True), full_matrices=False)
    # Per-conformation alignment via inertia tensor
...
class PolymerDataset(Dataset):
    def __init__(self, data_path, train=True, train_fraction=0.8):
        ...
        coords = data[list(data.keys())[0]]  # shape: [N, 12, 2] or similar
        if coords.ndim == 2:
...
def train(data_dir="data/polymer", output_dir="outputs/polymer_vae"):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train Polymer VAE")
    parser.add_argument("--data_dir", type=str, default="data/polymer")
    parser.add_argument("--output_dir", type=str, default="outputs/polymer_vae")
